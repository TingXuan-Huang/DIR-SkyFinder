"""Training loop for the SkyFinder DIR experiments.

This module only holds the orchestration -- `run_baseline(cfg)`. The model
factories, Dataset, eval helpers, and checkpoint I/O all live in
`dir_skyfinder/baseline.py` (which is now a pure-library module with no driver).

Usage:
    from dir_skyfinder.trainer import run_baseline
    results = run_baseline(model="resnet50", epochs=20)

For YAML-driven experiment runs, see `run.py`.
"""
...
from dir_skyfinder.lds import MIN_TEMP, compute_lds_weights
...
def run_baseline(cfg: Config | None = None, save: bool = True, **kwargs) -> dict:
    ...
    if save and cfg.snapshot_every > 0 and start_epoch == 0:
        save_checkpoint(net.state_dict(), f"{cfg.run_name}_ep0")
    ...
    for ep in range(start_epoch, cfg.epochs):
        ...
        if save and cfg.snapshot_every > 0 and (ep + 1) % cfg.snapshot_every == 0:
            save_checkpoint(net.state_dict(), f"{cfg.run_name}_ep{ep + 1}")
